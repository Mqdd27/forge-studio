...
export function WorkCard({ item, index = 0 }: { item: Pick<(typeof cases)[number], "key" | "slug" | "visual">; index?: number }) {
  ...
  return (
    <Link
      href={`/work/${item.slug}`}
      className="group flex h-full min-w-0 flex-col bg-[#FFFDF7] transition-colors duration-150 hover:bg-[#111111] hover:text-[#F3F0E9]"
    >
      <div className="flex items-center justify-between border-b border-[#111111]/15 px-5 py-3">
        <span className="text-[11px] font-semibold tracking-[0.08em] text-[#A93100] uppercase">[{n}]</span>
        <span className="max-w-[60%] truncate text-[11px] font-semibold tracking-[0.08em] text-[#5F5E5E] uppercase group-hover:text-[#F3F0E9]">
      ...
      <div className="flex flex-1 flex-col p-5 md:p-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-[#5F5E5E] group-hover:text-[#B9B5AE]">{tCases(`${item.key}.summary`)}</p>

        <ul className="mt-4 flex list-none flex-wrap gap-1.5 p-0">
          {presentation.tags.slice(0, 4).map((tag) => (
            <li
              key={tag}
              className="border border-[#111111]/25 px-2 py-1 text-[10px] font-semibold tracking-[0.06em] uppercase group-hover:border-[#F3F0E9]/40"
            >
        ...
        <div className="mt-auto pt-6">
          <div className="flex items-center justify-between border-t border-[#111111]/15 pt-3 text-xs font-semibold tracking-[0.06em] text-[#A93100] uppercase group-hover:border-[#F3F0E9]/25">
